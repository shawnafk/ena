...
class prob():
    ...
    class ena:
        ...
        def sub_E(self,El,Eu):
            ...
            sub_ena = prob.ena(self.t[sub_ori_idx],self.xz[:,sub_ori_idx],self.pt[:,sub_ori_idx],self.E[sub_ori_idx])
            return sub_ena

        def sub_T(self,tl,tu):
            t = self.t
            sub_idx = (t >= tl) & (t <= tu)
            sub_ena = prob.ena(self.t[sub_idx],self.xz[:,sub_idx],self.pt[:,sub_idx],self.E[sub_idx])
            return sub_ena
